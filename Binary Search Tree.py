...
def delete_floor(floor_tree,floor):
    if floor.left==None:
        trans(floor_tree,floor,floor.right)
    elif floor.right==None:
        trans(floor_tree,floor,floor.left)
    else:
        
        y=mini(floor.right)
            
        if y.parent!=floor:
            trans(floor_tree,y,y.right)
            y.right=floor.right
            y.right.parent=y
        trans(floor_tree,floor,y)
        y.left=floor.left
        y.left.parent=y
...
def find_nearest_unhaunted(floor_tree,floor_num):
    if floor_tree.root==None:
        return floor_tree.root
    if floor_num==floor_tree.root.floor_num:
        return floor_tree.root
    else:
        x=floor_tree.root
        m_node=x
        m= abs(floor_num-x.floor_num)
...
floor_list6 = [6,19,7,1,14,17,3,10]
...
del_6 = corrls_6[1]
corrls_6d = [None,7,1,19,None,3,14,None,None,None,None,None,10,17]
    ...
    for i in range(len(testlists)):
        ...
        if enable_turtle:
            ...
            if actual != None:
                ...
                draw_elevator(ghosts,draw_turtle,target,0)
            floor_tree.draw_tree()
