...
def optimize_wings(price_list,num_wings):
    ...
    for i in range(len(price_list)):
        length.append(price_list[i].wings)
        relaprice.append(price_list[i].price)
    ...
    for j in range(1,num_wings+1):
        ...
        for i in range(1,j+1):
            if i in length :
                k= length.index(i)
                if best>relaprice[k]+rev[j-i]:
                    best=relaprice[k]+rev[j-i]
                    s[j]=i
        rev[j]=best


    rev2=[0]*(2*num_wings+1)
    s2=[0]*(2*num_wings+1)
    ...
    for j in range(num_wings,2*num_wings+1):
        best2=float('inf')    
        for i in range(1,j+1):
            if i in length :
                k= length.index(i)
                if best2>relaprice[k]+rev2[j-i]:
                    best2=relaprice[k]+rev2[j-i]
                    s2[j]=i
                  #  print(s2[j])
        rev2[j]=best2
 #   print(s[j])
 ...
 #   print (s2)         
    ...
    new=[]
    cheapest=rev[num_wings]
  #  print(cheapest)
    for i in range(num_wings+1,len(rev2)):
        if rev2[i]<cheapest:
            cheapest=rev2[i]
    
    cheapnum=rev2.index(cheapest)
  #  print(cheapnum)
   # print(num_wings)
    
    
    if cheapnum>num_wings:  
        while cheapnum>0:
            k=s2[cheapnum]
            for i in price_list:
                if i.wings==k:
